...
CLIENTE_PASAPORTE AS PASAPORTE,
CONCAT(CLIENTE_NOMBRE,' ',CLIENTE_APELLIDO) AS NOMBRE_Y_APELLIDO,
COUNT(CLIENTE.NACIONALIDAD_ID) OVER (PARTITION BY NA.NACIONALIDAD_DESCRIPCION) AS TOTAL_DE_INMIGRANTES
FROM CLIENTE
INNER JOIN NACIONALIDAD NA ON CLIENTE.NACIONALIDAD_ID=NA.NACIONALIDAD_ID
)
SELECT 
NACIONALIDAD,PASAPORTE,NOMBRE_Y_APELLIDO
FROM DUPLICADOS
ORDER BY TOTAL_DE_INMIGRANTES DESC;
